    ...
    for length1 in np.arange(a_min, a_max, 0.5):
        for length2 in np.arange(a_min, a_max, 0.5):
            ...
            for i in range(1,11):
                ...
                #plt.show()
                count += 1
            samples_train.extend(samples_temp[0:5])
            labels_train.extend(labels_temp[0:5])
            samples_validation.extend(samples_temp[5:8])
            labels_validation.extend(labels_temp[5:8])
            samples_test.extend(samples_temp[8:10])
            labels_test.extend(labels_temp[8:10])
            

    print(f"{count} datas has been generated")

#save_h5('standard_dataset_tetragonal_random_30intlorentz.h5',samples,labels)
save_h5('standard_trainset_tri_hxagonal_5intlorentz.h5',samples_train,labels_train)
save_h5('standard_validationset_tri_hxagonal_3intlorentz.h5',samples_validation,labels_validation)
save_h5('standard_testset_tri_hxagonal_2intlorentz.h5',samples_test,labels_test)
